...
- Preserve the explicit injection API: `fortenv.string((secrets, ...businessArguments) => result)` (the `Fortenv` class and default `fortenv` instance; secret keys are typed via a `SecretValues<Keys>` parameter annotation or `new Fortenv<Keys>()`, both erased typing aids; `.buffer`/`.secret` delivery forms forthcoming). All direct reads of configured keys through `process.env` are denied, including inside registered callbacks.
