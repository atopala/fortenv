...
| ID / status                                                        | Attack experiment                                                                                                                                                                                                                                              | Secure result and possible fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [x] SEC-09 — fixed (config); bootstrap name-comparison held        | At T1 patch reflection, descriptor access, array validation, map membership, or name-comparison operations used by `configuration.ts` and `bootstrap.ts`. Attempt admitting an invalid target or mismatched secret set through the real validation path.       | Actual validation is enforced and bootstrap does not become ready on failure. Explore trusted validation operations. Trusted config becoming nondeterministic is a separate limitation: a mismatch check cannot undo a read of a key discovery never protected. **Disposition:** `configuration.ts` routed through captured `Array.isArray`/append/traversal, `Reflect.ownKeys`, `getOwnPropertyDescriptor`, `getPrototypeOf`, and Map ops; covered by `03-config-registration/{array-operations,property-descriptor,map-iterator}`. The `bootstrap.ts` name-comparison (`03-config-registration/name-comparison`) held: `matchingNames` is layered (size → membership → case-uniqueness), so neutralizing the `.some` membership predicate does not bypass it — the size check fails closed with `FortenvConfigError` first, and a real-only name was never captured so it is absent from the value map regardless. |
...
- **SEC-10a — guard-handler and wrapper-handler immutability (fix technique).** As a defense-in-depth fix rather than an attack, evaluate freezing the `process.env` guard handler object and the `fortenv()` wrapper Proxy handler after install so a dependency cannot mutate individual trap functions post-installation. Confirm this does not change trap behavior or the public contract before adopting it. This does not replace capturing the traps' own `Reflect`/`filter` operations under SEC-10. **Status — not implemented (by design):** the guard and wrapper handler objects are module-scoped closures with no external reference (SEC-24 confirmed external code cannot reach them), so freezing them defends a threat that requires a reference no code can obtain. It would be speculative code with no security benefit and is intentionally omitted.

### Priority 2 — Broader boundary checks and documented limits

| ID / status                        | Attack experiment                                                                                                                                                                                                                                                                                                       | Secure result and possible fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [~] SEC-14 — held                  | Enumerate public exports; attempt package subpath and resolvable direct-file imports; load a second runtime copy where feasible. Inspect exported helpers and exposed objects for a path to the live store or grant installation.                                                                                       | No import exposes live private state or a registration backdoor. Export maps alone are not a security boundary against arbitrary filesystem paths. Standalone helpers operating on caller-supplied data are not a bypass. Explore removing live-state access if found; retain the existing duplicate-guard rejection. **Disposition:** held (`07-package-boundaries/export-surface`). Deep scan of all public namespaces finds no secret/live store; internal dist subpath imports are refused by the exports map; no grant-installation backdoor; root exports are exactly the intended API.     |
| [~] SEC-15 — held                  | Extend discovery fixtures with syntax variations around comments, strings, template literals, import attributes, re-exports, and dynamic import expressions supported or rejected by the current design. Use an imported module with an execution marker, then native Node execution as the supported-syntax reference. | Discovery does not execute application imports; unsupported constructs fail as documented rather than silently omitting required protection. This is a bounded transformer correctness investigation, not a hostile-config sandbox project. Parser expansion or new supported syntax needs approval. **Disposition:** held (`07-package-boundaries/discovery-execution` at integration level; `13-pipeline/phase-1-discover` at unit level). An imported side effect runs only in phase-2 real import, after the guard, so it never captures the value pre-protection.                            |
| [~] SEC-16 — held                  | Probe retained original environment references, ESM/CommonJS process imports, ordinary child inheritance, and separately initialized workers using explicit fake secrets.                                                                                                                                               | Sanitized references and normal child environments do not reveal captured values. Workers have separate setup; do not add automatic secret/grant propagation. Reuse existing guard/consumer tests before adding new fixtures. **Disposition:** held (`09-environment-reacquisition/routes`). `process.env`, `globalThis.process.env`, `require("node:process").env`, and `import { env }` all deny the protected read; a spawned child does not inherit the scrubbed value. Pre-load references and workers are documented limitations (SEC-17).                                                  |
| [~] SEC-17 — documented limitation | Document T0 tampering, Linux initial environment retention, explicit credential handoff, and calls to accessible wrappers. Optional isolated demonstrations use only fake values and never log their content.                                                                                                           | Clearly label these as prerequisites or design limitations, not tests that should turn green after built-in capture. Process isolation or a launcher is separate future design work, not an automatic addition to this implementation. **Disposition:** documented (`12-limitations/CONTEXT.md`) with a T0 boundary control (`12-limitations/preload-capture`): a pre-load import reads the raw value (limitation) while the post-load ambient read is denied (guarantee). `/proc/self/environ`, explicit handoff, accessible wrappers, and shared-heap access are documented, not fixable in V1. |
