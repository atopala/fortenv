...
- `intrinsics.ts` captures the object creation/freezing, JSON formatting, reflection, array and collection operations currently used on secret-bearing injection records, configuration copies, errors, the private value map, and the grant registry: array detection, append and traversal; own-key, descriptor and prototype lookup; `Object.create`/`Object.freeze`; Map construction, membership, reads, writes and entry traversal; WeakMap construction, reads and writes; WeakSet membership and registration; Set construction, membership, writes and traversal; and the `isGeneratorFunction` check used to validate wrapped functions. `security-errors.ts` captures `Error.captureStackTrace` and makes stack capture best-effort so building a Fortenv error never throws. Prototype methods and iterator advancement run through captured `Reflect.apply`. Tests in group 15 verify that replacements installed during config loading or after bootstrap cannot intercept those values, rewrite the tested config grants, forge a runtime lookup, capture or redirect the grant registry, expand an installed grant, register a raw function as a wrapper, disable the environment guard's protected-name lookup, replace the tested denial errors through JSON formatting, bypass the generator check by mutating the `node:util/types` binding, or break the denial error contract by replacing `Error.captureStackTrace`. Additional string normalization and reporting operations remain tracked by the hardening plan rather than implied safe by these focused fixes.
