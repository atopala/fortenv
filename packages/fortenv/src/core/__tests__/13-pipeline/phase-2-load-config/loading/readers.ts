...
export const readDatabase = fortenv.string(
   ({ DATABASE_URL, MISSING_SECRET }: SecretValues<"DATABASE_URL" | "MISSING_SECRET">) => {
...
export const readBoth = fortenv.string(
   ({ DATABASE_URL, STRIPE_SECRET_KEY }: SecretValues<"DATABASE_URL" | "STRIPE_SECRET_KEY">) => [
      DATABASE_URL,
      STRIPE_SECRET_KEY,
   ],
);
export const readStripe = fortenv.string(({ STRIPE_SECRET_KEY }: SecretValues<"STRIPE_SECRET_KEY">) => {
   assert.throws(() => process.env.STRIPE_SECRET_KEY, /unauthorized access.*STRIPE_SECRET_KEY/);
