# ESM consumer with ESM dependency

`integration.test.ts` launches these real applications using the built Fortenv preload. The consumer's `.js` files use import; the dependency is the private `@fortenv-fixture/esm-dependency` workspace package.
