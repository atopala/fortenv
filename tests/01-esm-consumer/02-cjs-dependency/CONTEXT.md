# ESM consumer with CJS dependency

`integration.test.ts` launches these real applications using the built Fortenv preload. The consumer's `.js` files use import; the dependency is the private `@fortenv-fixture/cjs-dependency` workspace package.
