# ESM Fortenv consumer

Private integration project with `"type": "module"`. Applications use native ESM imports and consume Fortenv's built package exports. Config remains ESM `fortenv.config.mjs`; `.cjs` configs are not added to the library contract.

| Folder                                            | Coverage                                                       |
| ------------------------------------------------- | -------------------------------------------------------------- |
| [01-esm-dependency](01-esm-dependency/CONTEXT.md) | ESM consumer → actual ESM package                              |
| [02-cjs-dependency](02-cjs-dependency/CONTEXT.md) | ESM consumer → actual CommonJS package                         |
...
```sh
pnpm --dir tests/01-esm-consumer test
pnpm test:modules
```
